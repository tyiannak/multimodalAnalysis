...
if __name__ == '__main__':
    f1, _, fn1 = aF.dirWavFeatureExtraction("/Users/tyiannak/ResearchData/Audio Dataset/musicalGenreDatasets/dortmund/alternative", 1, 1, 0.1, 0.1)
    f1 = np.array([f1[:, fn1.index(name_1)], f1[:, fn1.index(name_2)]]).T
    f2, _, fn2 = aF.dirWavFeatureExtraction("/Users/tyiannak/ResearchData/Audio Dataset/musicalGenreDatasets/dortmund/blues", 1, 1, 0.1, 0.1)
    f2 = np.array([f2[:, fn1.index(name_1)], f2[:, fn1.index(name_2)]]).T
    f3, _, fn3 = aF.dirWavFeatureExtraction("/Users/tyiannak/ResearchData/Audio Dataset/musicalGenreDatasets/dortmund/electronic", 1, 1, 0.1, 0.1)
    ...
    f1 = (f1 - mean) / std; f2 = (f2 - mean) / std;  f3 = (f3 - mean) / std; f = (f - mean) / std
    plt1 = go.Scatter(x=f1[:, 0], y=f1[:, 1], mode='markers', name="1")
    plt2 = go.Scatter(x=f2[:, 0], y=f2[:, 1], mode='markers', name="2")
    plt3 = go.Scatter(x=f3[:, 0], y=f3[:, 1], mode='markers', name="3")

    y = np.concatenate((np.zeros(f1.shape[0]), np.ones(f2.shape[0]), 2*np.ones(f3.shape[0])))
